...
class PatientsListPage(BasePage):
    ...
    def open_patient_list_page(self):
        ...
        self.logger.debug("tests_patient list is opened")

    def click_create_patient_button(self):
        ceate_patient_button = self.driver.find_element(By.XPATH,
                                                        value=patient_list_constant.CREATE_PATIENT_BUTTON_XPATH)
        ceate_patient_button.click()
        self.logger.debug("create tests_patient button is clicked")
    ...
    def fill_unique_first_name_field(self, patient_first_name):
        ...
        self.logger.debug(f"unique tests_patient name is entered as {patient_first_name}")

    def fill_last_name_field(self, last_name):
        self.wait_until_send_keys(locator_type=By.XPATH, locator=patient_list_constant.LAST_NAME_FIELD_XPATH,
                                  data=last_name)
        self.logger.debug(f"unique tests_patient last name is entered as {last_name}")

    def input_valid_email_field(self, valid_patient_email):
        self.wait_until_send_keys(locator_type=By.XPATH, locator=patient_list_constant.EMAIL_FIELD_XPATH,
                                  data=valid_patient_email)
        self.logger.debug(f"unique tests_patient email is entered as {valid_patient_email}")
    ...
    def fill_phone_field(self, phone):
        ...
        self.logger.debug(f"unique tests_patient phone is entered = {phone}")
    ...
    def select_payment_option(self, payment_option):
        ...
        self.logger.debug("payment option is selected into the tests_patient creation overlay")

    def click_create_on_patient_creation_overlay(self):
        self.wait_until_click(locator_type=By.XPATH,
                              locator=patient_list_constant.CREATE_PATIENT_BUTTON_ON_POP_UP_XPATH)
        self.logger.debug("create tests_patient button is clicked on the creation overlay")

    def verify_patient_is_displayed_on_list(self, patient_first_name):
        list_of_patients = self.driver.find_elements(By.XPATH,
                                                     value=patient_list_constant.LIST_OF_PATIENTS_FIRSTMAMES_XPATH)
    ...
    def open_patient_overlay(self):
        ...
        """Click 'create tests_patient' button"""
    ...
    def search_created_patient(self, first_name):
        ...
        self.logger.debug(f"{first_name} of tests_patient is entered into the searchbox")
        time.sleep(1)

    def count_of_patients(self):
        time.sleep(2)
        pagination_element = self.driver.find_element(By.XPATH,
                                                      value="//div[@class='MuiTablePagination-root']/div/p[2]").text
        self.logger.debug(
            f"pagination element shows the following info = {pagination_element} and split = {pagination_element.split()}")
    ...
    def open_created_patient_displayed_on_list(self, patient_first_name):
        ...
        for each_patient in list_of_patients:
            if each_patient.text == patient_first_name:
                each_patient.click()
                self.logger.debug(f"created tests_patient is opened")
                break
    ...
    def verify_assigned_medico_is_displayed_on_list(self, patient_first_name, assigned_medico):
        list_of_patients = self.driver.find_elements(By.XPATH,
                                                     value=patient_list_constant.LIST_OF_PATIENTS_FIRSTMAMES_XPATH)
        for each_patient in list_of_patients:
            if each_patient.text == patient_first_name:
                assigned_medico_element = each_patient.find_element(By.XPATH, value=patient_list_constant.LIST_OF_PATIENTS_MEDICOS_XPATH)
                assert assigned_medico_element.text == assigned_medico
                self.logger.debug(f"{assigned_medico_element} is verified")
                break
